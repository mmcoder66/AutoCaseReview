...
# Email only for a specific iteration.
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--iteration", default=None,
                        help="所属迭代 filter; required for --mode email.")
    ...
    parser.add_argument("--version", default=None,
                        help="Release/version tag appended to every SVN filename "
                             "(e.g. v1.0.1, SP7).  Required for --mode svn|all; "
...
def _run_email(args, df_all) -> Path:
    ...
    df_email = load_all_requirements(args.data_dir, iteration=iteration)
    return gen_email(
        df_email,
...
def main() -> int:
    ...
    if args.mode == "email" and not args.iteration:
        raise SystemExit("--iteration is required when --mode=email")
    ...
    df_all = load_all_requirements(args.data_dir)
