...
│   │   ├── LC-SOP-RC-003-R01_*.docx # SVN Word 会议纪要模板
│   │   └── email.png                # 邮件样式参考图
...
| `生成用例评审会议纪要` | 读取 `inputs/requirement_data/` 下所有 xlsx | 先清空 `outputs/测试用例评审/` 和 `outputs/会议纪要/`，再按每条需求生成 Excel + Word | 是 |
...
| `LC-SOP-RC-003-R01_会议纪要_{product}_测试用例评审_{title}_{version}.docx` | 每条需求 1 份 | `outputs/会议纪要/` | SVN 归档，SOP 会议纪要 |
...
## 数据源
数据文件放在 `inputs/requirement_data/*.xlsx`。先从 Ones 按迭代筛选需求并导出 xlsx，再手动补充 `待办事项N` 列；整理完成后放入该目录。文件名必须带迭代关键字，例如 `SP7`、`SP8`，邮件生成会按该关键字筛选数据文件。
...
| `待办事项N` | 任意数量的待办列，如 `待办事项1`、`待办事项4`；脚本按 N 自动展开；列名任意位置有空格也能识别 |

生成前会自动校验数据。校验失败时不会生成文件；校验警告会继续输出，例如 `测试用例链接` 为空、未检测到待办列、待办列名格式不符合 `待办事项N`（如带有 `@责任人` 后缀或其他后缀）。

## 字段规则
- SVN Excel 只填 `测试用例评审记录` 页指定字段，以及签名页起草行的签名列；封皮、嵌入对象、图片和 13 项检查项结构保持模板原样。
- 签名页（嵌入 Word）只填起草行的签名列（值 = 当前需求行 `测试`），其他签字单元格（印刷体姓名、审核行、批准行等）保持空白。
- 签名页是 OLE 嵌入对象，Excel/WPS 默认显示模板原始预览图，不会实时渲染填入的签名；双击激活签名页或保存后由 Office 重生成预览图，才能看到填入的姓名。
- SVN Word 的 `记录人员` 默认取当前需求行 `测试` 字段；CLI 传入 `--recorder` 时可覆盖。
- SVN Word 的 `会议内容` 写入当前需求所有 `待办事项N` 原文；没有待办事项时填 `无`。
- SVN Word 的 `改进或遗留工作项` 按待办事项拆行，责任人从 `@姓名` 提取；没有待办事项时表格填 `无`。
- 邮件 Word 的需求表为 `编号 / 标题 / 测试 / 评审结果 / 备注#`，评审结果固定为 `通过`。
- 邮件 Word 的 `1.`、`2.`、`3.` 章节标题加粗；`3.评审记录/待办事项` 下的需求 ID 和需求标题不加粗，字号小于章节标题。
- 邮件 Word 的待办列表保留 `@责任人` 原文。
...
- 模板封皮、签名页结构、13 项检查项结构不得删除或重排；签名页只填起草行签名列，其他签字单元格保持空白。
- `@责任人` 原文必须保留在邮件和会议内容中。
